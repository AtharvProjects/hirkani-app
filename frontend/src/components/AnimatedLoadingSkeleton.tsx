...
const AnimatedLoadingSkeleton = () => {
    ...
    const getGridConfig = (width: number): GridConfig => {
        ...
        const cols = width >= 1024 ? 3 : width >= 640 ? 2 : 2 // Set columns based on screen width
        return {
            numCards,
            cols,
            xBase: 20, // Starting x-coordinate
            yBase: 40, // Starting y-coordinate
            xStep: 180, // Horizontal spacing
            yStep: 200 // Vertical spacing
        }
    }
                ...
                <div className="grid grid-cols-2 gap-4">
                    {[...Array(config.numCards)].map((_, i) => (
                        <motion.div
                            ...
                            className="bg-white/40 rounded-[24px] shadow-sm p-4 backdrop-blur-md border border-white/60"
                        >
                            {/* Card placeholders */}
                            <motion.div
                                className="h-24 rounded-[18px] mb-3"
                                animate={{
                                    background: ["rgba(243,244,246,0.5)", "rgba(229,231,235,0.7)", "rgba(243,244,246,0.5)"],
                                }}
